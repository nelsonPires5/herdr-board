...
live catalog is `e2e/README.md` (scenarios 01–26); `e2e/test-harness.sh` is the provider-free static
safety gate.
...
- End-to-end: `e2e/run-all.sh` (compat: `scripts/e2e.sh`) drives a REAL Herdr with a scenario
  ...
  (`--keep` leaves sessions/workspaces for review). The forced-build standard suite passes 01–26
