...
- [#TBD](https://github.com/nelsonPires5/herdr-board/pull/TBD) docs(visual-validation): require ALWAYS-isolated board state — DB, socket, AND daemon — under a short `/tmp` dir (no exceptions for trivial or "quick peek" runs; `board tui` auto-starts its own isolated daemon) and open runnable prototypes in a visible wezterm tab for interactive validation, matching the Prototyping-column prompt.
