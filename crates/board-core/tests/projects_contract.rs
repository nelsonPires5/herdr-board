...
fn fresh_db_has_only_the_global_project_with_a_main_board() {
    ...
    assert_eq!(db.recent_project_ids_excluding(None).expect("recents"), Vec::<i64>::new());

    let board = db.project_context_board(projects[0].id).expect("context board");
...
fn project_creation_selects_project_and_main_board() {
    ...
    assert_eq!(db.selected_project_id().expect("selected"), Some(project.id));
    assert_eq!(db.selected_board_id_for(project.id).expect("selected"), Some(board.id));
    assert_eq!(
        db.recent_project_ids_excluding(Some(project.id)).expect("recents"),
        Vec::<i64>::new(),
        "the current project is excluded from its own recents"
    );
    // Duplicate creation is a bad request.
    let dup = db.create_project_context("/tmp/alpha/project").expect_err("duplicate");
...
fn recency_is_capped_at_three_and_most_recent_first() {
    ...
    assert_eq!(recents, vec![projects[4].id, projects[3].id, projects[2].id]);
    ...
    let recents = db.recent_project_ids_excluding(Some(projects[0].id)).expect("recents");
    assert_eq!(recents, vec![projects[4].id, projects[3].id]);
    let recents = db.recent_project_ids_excluding(None).expect("recents");
    assert_eq!(recents, vec![projects[0].id, projects[4].id, projects[3].id]);
}
...
fn per_project_board_recency_and_selection_are_isolated() {
    ...
    let b3 = db.create_board(pb.id, "Backlog").expect("other project backlog");
    ...
    assert_eq!(db.selected_board_id_for(pb.id).expect("selected"), Some(b3.id));
    // Project A's board selection is untouched by B's activity.
    assert_eq!(db.selected_board_id_for(pa.id).expect("selected"), Some(b2.id));
}

#[test]
fn selecting_a_missing_project_fails_with_the_create_hint() {
    let db = mem();
    let err = db.require_project_by_scope("/never/created").expect_err("missing");
...
fn board_create_is_auto_selected_and_cannot_duplicate_names_case_insensitively() {
    ...
    let dup = db.create_board(project.id, "backlog").expect_err("case-insensitive dup");
    assert_eq!(dup.code(), 1);
...
fn open_board_resolution_never_touches_selection_or_recency() {
    ...
    assert_eq!(db.recent_project_ids_excluding(None).expect("recents"), before_recents);
...
fn project_list_result_is_deterministic_and_picker_ready() {
    ...
    db.select_project_by_scope("/r/alpha", None).expect("select alpha");

    let result = db.project_list_result().expect("list");
    // Folder-name order, Global last.
    let names: Vec<&str> = result.projects.iter().map(|p| p.project.name.as_str()).collect();
    assert_eq!(names, vec!["alpha", "zeta", "Global"]);
    assert_eq!(result.selected_project_id, result.projects[0].project.id.into());
