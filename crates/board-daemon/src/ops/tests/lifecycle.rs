//! The `run.done` / `pane_exited` acceptance matrix: which reports a queued or
//! active run accepts, which it refuses without mutating anything, and the
//! open-run guards on retry and column deletion.
