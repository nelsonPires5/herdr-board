...
fn project_open_result(d: &Arc<Daemon>, pair: (board_core::model::Project, board_core::model::Board)) -> Result<Value> {
...
pub(super) fn project_select(d: &Arc<Daemon>, p: ProjectSelectParams) -> Result<Value> {
    let pair = d.store.lock().select_project_by_scope(&p.scope_path, p.board_id)?;
    project_open_result(d, pair)
}
