...
every numbered scenario from **01 through 34** must appear here and in `run-all.sh`. The provider-free
...
scenarios 01–34 without provider calls; this is a coverage requirement, not a claim that a live run
...
| `run-all.sh` | Builds once, runs scenarios 01–34 as environment-scrubbed children with their own sessions, captures artifacts, and prints the summary (`--require-all` forbids skips). |
