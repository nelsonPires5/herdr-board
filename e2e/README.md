...
| Managed protocol-20/current Pi + Claude: pane-first placement, exact 0600 system file, readiness/session reports, exact `agent.prompt` task delivery, and held layout where each managed tab converges to exactly one harness pane (no anchor) | `16-managed-p17.sh` | live, checked-in fake `pi` + `claude`, zero provider cost; historical filename |
| Unmanaged protocol-20/current configured harness: exact argv/multiline env/cwd/socket bridge through CLI-only `pane run`, held layout, explicit completion | `17-configured-p17-runner.sh` | live, temporary runner, zero provider cost; historical filename |
...
| Adopt an already-running Herdr agent into the Global board, focus its exact pane, then cancel only the board link while the external pane remains alive | `40-adopt-existing-agent.sh` | live, checked-in fake `pi`, zero provider cost; external ownership boundary |

### How the live scenario produces Herdr `done`

Herdr 0.8.2 / protocol 20 exposes `done` as an output `AgentStatus`, but its
...
`e2e/ci.sh` downloads or reuses the exact SHA-verified Herdr 0.8.2 Linux x86_64 asset, verifies
protocol 20, invokes `run-all.sh --require-all`, and validates the one private artifact root printed
...
| `16-managed-p17.sh` | Managed pane-first Pi/Claude protocol-20/current launch, no-provider boundary, and anchorless managed tabs that converge to exactly one harness pane (historical filename). |
| `17-configured-p17-runner.sh` | Unmanaged protocol-20/current configured-command `pane run` bridge and exact argv/env evidence (historical filename). |
...
| `40-adopt-existing-agent.sh` | Links a verified live Pi pane as an external board run, focuses the exact pane without changing pane count, and proves board cancel finalizes only the link while leaving the external pane alive. |
