...
every numbered scenario from **01 through 35** must appear here and in `run-all.sh`. The provider-free
...
| Git-root/CWD board identity, independent pipelines/cards, scoped TUI title, Global picker entry, and deterministic auto-spawn cwd in a heterogeneous workspace | `12-cwd-boards.sh` | live |
...
scenarios 01–35 without provider calls; this is a coverage requirement, not a claim that a live run
...
| `12-cwd-boards.sh` | Scoped board identity/isolation, real TUI title/picker, and heterogeneous-workspace cwd override/fail-closed dispatch. |
...
| `run-all.sh` | Builds once, runs scenarios 01–35 as environment-scrubbed children with their own sessions, captures artifacts, and prints the summary (`--require-all` forbids skips). | with their own sessions, captures artifacts, and prints the summary (`--require-all` forbids skips). |
