...
| `crates/board-core/tests/{capability,config,prompt,harness,protocol,fake_client}.rs` | Harness catalog + pane-name slug rules; typed `RootConfig` board/daemon defaults and fatal parsing; prompt assembly + effective-settings; harness argv/session planning; protocol serde round-trips (including additive active-run summaries); the in-memory `FakeBoardClient`. |
