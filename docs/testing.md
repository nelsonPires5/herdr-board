...
| `crates/board-core/tests/db_atomic.rs` | File-backed abort-trigger rollback/reopen checks for enqueue, promotion, finalization, and auto-hop; a subprocess hard-exit fault between an internal statement and commit with zero returned effect/event; schema-v8 duplicate rejection and exact partial-index SQL. |
