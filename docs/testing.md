...
| `board-daemon::supervisor` tests | Scripted resolver/runtime probes enforce conservative restart classification: unresolved sessions and runtime timeout/malformed/panic failures are `Unknown`, only a successful missing-pane snapshot is `Gone`, and `Alive` adoption is revalidated and idempotent before restoring watch/status intent. The always-on T10 stream is intentionally outside this layer. |
