...
| `crates/board-core/tests/db_atomic.rs` | File-backed abort-trigger rollback/reopen checks for enqueue, promotion, finalization comments/transitions, and auto-hop; a subprocess hard-exit fault between an internal statement and commit with zero returned effect/event; schema-v8 duplicate rejection and exact partial-index SQL. Daemon tests additionally inject comment and next-enqueue failures, reopen the DB to compare the exact prior run/card/comments, assert no pre-commit kill/event/wake, cover the board-done/cancel/timeout/pane-exit duplicate-and-stale winner matrix, and record the exact post-commit effect order. |
