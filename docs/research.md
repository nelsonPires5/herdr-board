...
## A. Herdr capability map (Herdr 0.9.0, socket protocol 22; local introspection)

JSON request/response and events use the Unix socket at
`~/.config/herdr/herdr.sock` (or `HERDR_SOCKET_PATH` for a named session).
The captured `herdr api schema --json` contains **102 request methods, 26 emitted event
...
| Need | Herdr 0.9.0 command / current socket API |
|---|---|
...
| Capture a reported session | `agent.get` → `AgentInfo.agent_session` (`AgentSessionInfo | null` in the protocol-22 schema): `{agent, kind, source, value}` with `kind` ∈ {`id`, `path`}; the field is absent when no session was reported. The codex integration reports its self-minted thread id here and the opencode integration its `ses_…` session id; board-herdr decodes the field and the daemon's bounded post-launch capture accepts only the expected agent + `kind:"id"` + non-empty `value` (opencode additionally pins the integration's `source`), promoting it atomically onto run+card. |
...
### 0.9.0 behavior that shaped live validation
...
**Agent status and integrations**: the installed 0.9.0 CLI exposes the integration
