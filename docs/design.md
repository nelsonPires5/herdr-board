...
- **Compact help and pickers:** the two-column help layout ellipsized nearly every description below ~80 columns, so Compact renders help as one entry per row with the full description, `j`/`k` scroll plus a scrollbar; Regular/Wide keep the existing two-column layout. Compact picker options wrap instead of clipping.
