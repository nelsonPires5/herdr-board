...
boardd supports **exactly Herdr 0.8.2 / protocol 20**. The board protocol remains v1 and the
...
- `run.focus {card_id, run_id, origin_socket}` →
  ...
  **`BOARD_RUN_ID` is deliberately cleared to empty** (the CLI treats empty as unset). It is not a label but the *actor credential*:
  ...
  `agent_name_taken` exclusivity backstop. Verified live against Herdr 0.8.2 / protocol 20: `agent.start` leaves a
...
- `pane.set_title {pane_id, title, origin_socket}` → `{renamed:true}` — set the label Herdr renders
  ...
  pinned Herdr 0.8.2 / protocol 20 check runs before the rename. Maps to herdr `pane.rename`, and
