...
- `harness.list` (no params) → `{harnesses:[…]}` — every harness the daemon knows about: the built-ins `pi`/`claude` plus every config-defined `[harness.NAME]`, sorted and de-duplicated. Drives the TUI harness/harness-override selects so config-defined harnesses are selectable without a client-side config read.
