...
Herdr **0.8.2 / socket protocol 20**. The versioned source of truth is `schema.sql` for fresh
...
reuse that live child: placement protects it from reclaim, waits for protocol-20 `Idle` or derived
...
- A: unit tests: engine transitions (ok/fail/no-target/manual entry), lifecycle decisions
  ...
  config parsing.
- B: unit: envelope encode/decode. Integration (ignored-by-default `#[ignore]` + run when HERDR_SOCK exists): read-only calls `session.snapshot`, `workspace.list` against a Herdr 0.8.2 / protocol-20 live socket.
...
- E: scenarios `e2e/01-core.sh` through `e2e/39-managed-slow-provider.sh` (real Herdr 0.8.2 / socket
  protocol 20, fake harnesses): disposable workspaces, pane-first placement, typed prompt delivery,
